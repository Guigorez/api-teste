...
# --- CONFIGURAÇÃO ---
BASE_DIR = os.getcwd()
PASTA_ENTRADA = os.path.join(BASE_DIR, 'Dados', 'Novoon', 'planilhas', 'Madeira Madeira')
PASTA_SAIDA = os.path.join(BASE_DIR, 'Dados', 'Novoon', 'planilhas limpas')
ARQUIVO_SAIDA = 'madeira_novoon_final.csv'
...
def processar_madeira():
    print("="*80)
    print("PROCESSANDO MADEIRA MADEIRA NOVOON (REFACTORED)...")
    print("="*80)
    ...
    if 'Status' in df_full.columns:
        ...
        df_full = df_full[~df_full['Status'].astype(str).str.contains('Cancelado', case=False, na=False)]
        print(f"ℹ️  Removidos {total_antes - len(df_full)} registros cancelados.")

    # --- SELEÇÃO E RENOMEAÇÃO ---
    mapa_colunas = {
        'Pedido': 'Id do Pedido Unificado',
        'Valor Total': 'Faturamento',
        'Valor Frete': 'Frete',
        'Valor Comissão': 'Comissoes',
        'Valor Liquido': 'Lucro Bruto', # Madeira chama de Liquido o que sobra pro seller
        'Produto': 'Produto',
        'Cidade': 'Cidade',
        'UF': 'UF',
        'Status': 'Status',
        'Data Pedido': 'Data Venda'
    }
    ...
    # --- CONVERSÃO NUMÉRICA ---
    cols_num = ['Faturamento', 'Frete', 'Comissoes', 'Lucro Bruto']
    for col in cols_num:
    ...
    # --- CÁLCULOS ---
    # Se comissão vier zerada, tenta calcular
    if df_clean['Comissoes'].sum() == 0:
        # Lucro = Faturamento - Comissao - Frete (geralmente)
        # Então Comissao = Faturamento - Lucro - Frete (se frete for custo positivo na planilha)
        # Mas vamos assumir o padrão Novoon:
    ...
    print("\n" + "="*80)
    print("RESULTADO MADEIRA MADEIRA FINAL:")
